...
struct `HTTP.Request.Line Tests` {
    ...
    struct Unit {
        ...
        func `Parse custom method`() async throws {
            ...
            #expect(parsed.method.description == "CUSTOM")
        }
    }
    ...
    struct Integration {
        @Test
        func `Format then parse round-trips`() async throws {
            let line = RFC_9110.Request.Line(
                method: .get,
            ...
            let reparsed = try RFC_9110.Request.Line.parse(line.formatted)
            #expect(reparsed.method == line.method)
            #expect(reparsed.target == line.target)
            #expect(reparsed.version == line.version)
        }
    }
}
